...
export const LOCK_DEFINITIONS = [
  {
    ...
    getLocked: (project) => Boolean(project?.brandName || project?.name),
    getValue: (project) => project?.brandName || project?.name || '未锁定',
  },
  {
    id: 'brief',
    label: '需求档案',
...
export const PHASE_WORKLIST = {
  1: {
    ...
    tasks: [
      task('p1-describe-need', '在设计工作台中描述需求', true),
      task('p1-upload-reference', '上传参考图/Logo 等素材', true),
      task('p1-confirm-brand', '确认品牌名称和基本方向', true),
...
function buildFacts(project) {
  ...
  return {
    ...
    hasBrandName: Boolean(project?.brandName || project?.name),
