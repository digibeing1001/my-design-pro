...
function buildCustomImageProvider(model = {}) {
  return {
    id: model.id,
    name: model.provider || model.name || '自定义渠道',
    modelId: model.id,
    modelName: model.name || model.model || '自定义画面生成',
    provider: model.provider || '自定义渠道',
    icon: model.icon || 'CU',
    region: '自定义',
    desc: model.desc || '团队自建或第三方画面生成渠道',
    ...
    fields: [
      ...
      textField('baseUrl', '服务地址', 'https://your-studio.example/v1', '填写官网或团队提供的服务地址'),
      textField('model', '生成器名称', 'provider/generator-id'),
    ...
    capabilities: capabilityProfile({
      ...
      strengths: [
        { zh: '自定义渠道', en: 'Custom channel' },
        { zh: '团队入口', en: 'Team entry point' },
      ],
      guidanceZh: '能力取决于团队渠道配置。交付前必须由源稿检查确认格式与授权。',
      guidanceEn: 'Capabilities depend on the team channel. Confirm format and rights before delivery.',
...
export const IMAGE_PROVIDER_PRESETS = [
  ...
  },
  {
    ...
    fields: [
      secretField('apiKey', '官网授权码', '粘贴官网授权码'),
      textField('baseUrl', '服务地址', 'https://your-studio.example/v1', '填写本机伙伴或团队提供的服务地址'),
      textField('model', '生成器名称', 'provider/generator-id'),
      textField('size', '默认画幅', '1024x1024'),
    ],
  },
...
const IMAGE_PROVIDER_CAPABILITY_PRESETS = {
  ...
  fal: capabilityProfile({
    role: 'gateway',
    outputs: [{ zh: '随所选渠道变化', en: 'Depends on selected channel' }],
    strengths: [
      { zh: '快速切换', en: 'Fast switching' },
      { zh: '多渠道汇聚', en: 'Multi-channel access' },
    ],
  ...
  'custom-openai-compatible': capabilityProfile({
    ...
    guidanceZh: '能力取决于本机创作伙伴或团队渠道。默认按概念图处理，除非渠道返回可编辑源文件。',
    ...
    handoffZh: '交付前必须由渠道声明格式，并通过源稿与品牌套件检查。',
